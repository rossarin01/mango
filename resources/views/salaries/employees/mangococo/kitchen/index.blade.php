            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-header">
                        <a href="{{ route('salaries.employees.mangococo.kitchen.create') }}" class="btn btn-primary">Add</a>
                        <!-- ปุ่มลบหลายรายการ -->
                        <form id="bulk-delete-form" action="{{ route('salaries.employees.mangococo.kitchen.bulkDelete') }}" method="POST" style="display:inline;">
                            @csrf
                                            ...
                                            <tr>
                                                ...
                                                <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                                                    <label class="btn btn-label-success" for="btnradio1">
                                                        <a href="{{ route('salaries.employees.mangococo.kitchen.edit', $salary->id) }}"></a>
                                                        <i class="tf-icons ti ti-pencil"></i>
                                                    </label>
                                                    <label class="btn btn-label-danger" for="btnradio2">
                                                        <form action="{{ route('salaries.employees.mangococo.kitchen.destroy', $salary->id) }}" method="POST" style="display:inline;">
                                                            @csrf
                                                            @method('DELETE')
...
<script>
    ...
    function editSelected() {
        ...
        if (selectedEmployees.length === 1) {
            var editUrl = "{{ route('salaries.employees.mangococo.kitchen.edit', ':id') }}";
