                    ...
                    <div class="card-header">
                        <div class="row mt-3">
                            ...
                            <div class="col-6" style="text-align: right">
                                <a href="{{ route('roster.create') }}" class="btn btn-primary">Add</a>
                            </div>
                        </div>
...
<script>
    ...
    $(document).on('click', '.btn_edit', function(e){
        e.preventDefault();
        let rostert_id =$(this).data('roster_id');
        window.location.href= '{{ url("rosteredit") }}/'+rostert_id;
    });
    ...
    $(document).on('click', '.btn_copy', function(e){
        ...
        let rostert_id =$(this).data('roster_id');
        Swal.fire({
            title: 'Want to copy roster data?',
            text: "You are copy roster data.!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: '{{ route("roster.copy") }}',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    dataType: 'json',
                    data: {
                        'rostert_id' : rostert_id
                    },
    ...
    $(document).on('click', '.btn_export', function(e){
        e.preventDefault();
        let rostert_id =$(this).data('roster_id');
        window.location.href= '{{ url("roster/export") }}/'+rostert_id;
    });
    ...
    $(document).on('click', '.btn_delete', function(e){
        ...
        let rostert_id =$(this).data('roster_id');
        Swal.fire({
            title: 'Want to delete roster data?',
            text: "You are deleting roster data.!",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
