                    ...
                    <div class="card-header">
                        <div class="row mt-3">
                            ...
                            <div class="col-6" style="text-align: right">
                                <a href="{{ route('rostertemplate.create') }}" class="btn btn-primary">Add</a>
                            </div>
                        </div>
...
<script>
    ...
    $(document).ready(function () {
        oTable = $('#dataTable').DataTable({
            ...
            responsive: true,
            ajax: {
                url: "{{ route('rostertemplate.datatable') }}",
                data: function(d) {
    ...
    $(document).on('click', '.btn_edit', function(e){
        e.preventDefault();
        let rostertemp_id =$(this).data('rostertemp_id');
        window.location.href= '{{ url("rostertemplateedit") }}/'+rostertemp_id;
    });
    ...
    $(document).on('click', '.btn_delete', function(e){
        ...
        let rostertemp_id =$(this).data('rostertemp_id');
        Swal.fire({
            title: 'Want to delete roster template data?',
            text: "You are deleting roster template data.!",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'POST',
                    url: '{{ route("rostertemplate.delete") }}',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    dataType: 'json',
                    data: {
                        'rostertemp_id' : rostertemp_id
                    },
